...
** Last update Sun Jun  8 21:36:04 2014 maillot
*/
...
static int	init_raytracer_img(t_all *data, t_windows *win, t_mlximg *img)
{
  if ((img->img_ptr = mlx_new_image(win->mlx_ptr, data->global.size_x,
				    data->global.size_y)) == NULL ||
...
static int	init_raytracer_win(t_all *data, t_windows *win)
{
  ...
  win->win_ptr = mlx_new_window(win->mlx_ptr, data->global.size_x,
				data->global.size_y, WIN_NAME);
  if (win->win_ptr == NULL)
    {
